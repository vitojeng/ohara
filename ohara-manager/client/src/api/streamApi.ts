...
const streamApi = new API(RESOURCE.STREAM);

export const create = (params: ServiceBody) => {
  return streamApi.post<ClusterResponse>({ name: params.name, body: params });
};

export const update = (params: ServiceBody) => {
  return streamApi.put<ClusterResponse>({
    name: params.name,
    group: params.group,
...
export const remove = (objectKey: ObjectKey) => {
  return streamApi.delete<BasicResponse>(objectKey);
};

export const get = (objectKey: ObjectKey) => {
  return streamApi.get<ClusterResponse>({
    name: objectKey.name,
...
export const getAll = (queryParams?: Record<string, unknown>) => {
  return streamApi.get<ClusterResponseList>({ queryParams });
};

export const start = (objectKey: ObjectKey) => {
  return streamApi.execute<BasicResponse>({ objectKey, action: COMMAND.START });
};

export const stop = (objectKey: ObjectKey) => {
  return streamApi.execute<BasicResponse>({ objectKey, action: COMMAND.STOP });
};

export const forceStop = (objectKey: ObjectKey) => {
  return streamApi.execute<BasicResponse>({
    objectKey,
    action: COMMAND.STOP,
