...
import useAddWorkerNode from 'hooks/useAddWorkerNode';
import { Node } from 'types';

interface AddWorkerNodeDialogProps {
  workerName: string;
  open: boolean;
  onClose: () => void;
}
...
const AddWorkerNodeDialog: React.FC<AddWorkerNodeDialogProps> = ({
  open,
  onClose,
  workerName,
}) => {
  const { data: worker, isLoading: isWorkerLoading } = useWorker(workerName);
  const { data: nodes, isLoading: isNodesLoading } = useNodes();
  const [addWorkerNode] = useAddWorkerNode();

  const nodesToBeSelected: Node[] = useMemo(() => {
    if (worker?.nodeNames) {
      return filter(
        nodes,
        (node) => !includes(worker.nodeNames, node.hostname),
      );
    }
    return [];
  }, [worker, nodes]);
  ...
  async function handleSubmit(values: FormData): Promise<void> {
    ...
    if (worker) {
      await addWorkerNode({ name: worker.name as string, nodeName });
    }
    onClose();
  }
    ...
    <Form
      ...
      render={(props: FormRenderProps<FormData>): React.ReactNode => (
        <Dialog
          ...
          loading={isWorkerLoading || isNodesLoading}
          onClose={onClose}
...
export default AddWorkerNodeDialog;
